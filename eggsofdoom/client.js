...
var CONFIG;
require(["eggsofdoom/config"], function(config) {
    config.CONFIG.gamedep = config.CONFIG.game_package + "/game";
    CONFIG = config.CONFIG;
    require(["communication", "websocktransport", "world", "input", "eggsofdoom/game", "ui"],
        function (communication, websocktransport, world, input, game, ui) {
            "use strict";
            ...
            var local_player_settings = null,
                ...
                _init = function () {
                    ...
                    var web_socket = new window.WebSocket('ws://' + CONFIG.connect_to_address + ':' + CONFIG.connect_to_port + '/'),
                        player_name = window.location.search.split("?screen_name=")[1];
                    // Set up settings for the local player
                    local_player_settings = {
                        name: null,
                        keys: {
                            37: {'down': game.INPUT_COMMANDS.LEFT_DOWN, 'up': game.INPUT_COMMANDS.LEFT_UP},
                            39: {'down': game.INPUT_COMMANDS.RIGHT_DOWN, 'up': game.INPUT_COMMANDS.RIGHT_UP},
                            38: {'down': game.INPUT_COMMANDS.UP_DOWN, 'up': game.INPUT_COMMANDS.UP_DOWN},
                            40: {'down': game.INPUT_COMMANDS.DOWN_DOWN, 'up': game.INPUT_COMMANDS.DOWN_UP},
             ...
             */
                _waitForConnectionOpen = function (web_socket) {
                    ...
                    web_socket.onopen = function () {
                        ...
                        _ui.setClickCallback(function (click) {
                            if (click === 0) {
                                input_device.doKeyUp({
                                    preventDefault: function () {
                                    }, keyCode: local_player_settings.keys.left
                                });
                                input_device.doKeyUp({
                                    preventDefault: function () {
                                    }, keyCode: local_player_settings.keys.right
                                });
                            } else if (click < 0) {
                                input_device.doKeyUp({
                                    preventDefault: function () {
                            ...
                            input_device.doKeyDown({
                                preventDefault: function () {
                                }, keyCode: local_player_settings.keys.start
                            });
                            input_device.doKeyUp({
                                preventDefault: function () {
                                }, keyCode: local_player_settings.keys.start
                ...
                _registerForLobbyStatePackets = function (web_socket) {
                    ...
                    web_socket.registerReceivedPacketCallback(communication.PACKET_TYPES.LOBBY_STATE, null, function (packet) {
                        ...
                        if (packet.prepare_for_start !== undefined) {
                            _ui.hideKeyinfoBox();
                            if (packet.prepare_for_start > 0) {
